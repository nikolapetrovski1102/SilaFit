...
#   SILEN_SSH    SSH target      (default: root@sila.fitness)
#   REMOTE_DIR   path on server  (default: /opt/silen)
#   DOMAIN       public host     (default: sila.fitness)
###############################################################################
set -euo pipefail

# ---- config -----------------------------------------------------------------
SILEN_SSH="${SILEN_SSH:-root@sila.fitness}"
REMOTE_DIR="${REMOTE_DIR:-/opt/silen}"
DOMAIN="${DOMAIN:-sila.fitness}"
...
# nginx serves this directory directly (see nginx-sila.fitness.conf), so
...
# ---- 3. sync deploy assets (never touches server-only/generated files) ------
# --delete makes the server mirror this directory, so anything that only exists
# on the server MUST be excluded or it is removed on every publish. That has
# already bitten us once: run-*.sh (generated by deploy.sh), compose.override.yaml
# (server-specific AllowedHosts from the migration), secrets/, tde-cert-backup/
# and uploads/ (user-uploaded split images, written by the API at runtime) are
# all server-side and are not in the repo.
log "Uploading deploy/ -> $SILEN_SSH:$REMOTE_DIR/deploy"
rsync -az --delete \
  --exclude '.env' --exclude 'publish.sh' --exclude '.DS_Store' --exclude '._*' \
  --exclude 'run-*.sh' --exclude 'compose.override.yaml' \
  --exclude 'secrets' --exclude 'tde-cert-backup' --exclude 'uploads' \
